    ...
    ui(new Ui::ResetDialog)
{
    ...
    connect(ui->return_btn,&QPushButton::clicked,this,&ResetDialog::sigreturnlog);
...
void ResetDialog::on_varify_btn_clicked()
{
    ...
    HttpMgr::GetInstance()->PostHttpRep(QUrl(gate_url_prefix+"/get_varifycode"),
                                        json_obj, Repid::ID_GET_VARIFY_CODE,Moduls::RESETMOD);
}

void ResetDialog::slot_reset_mod_finish(Repid id,QString res,Error err)
{
...
{
    //注册获取验证码回包逻辑
    _handlers.insert(Repid::ID_GET_VARIFY_CODE, [this](QJsonObject jsonObj){
    ...
    //注册注册用户回包逻辑
    _handlers.insert(Repid::ID_RESET_PWD, [this](QJsonObject jsonObj){
        int error = jsonObj["error"].toInt();
        if(error != Error::SUCCESS){
            showTip(tr("参数错误"),false);
            return;
...
void ResetDialog::on_sure_btn_clicked()
{
    ...
    HttpMgr::GetInstance()->PostHttpRep(QUrl(gate_url_prefix+"/reset_pwd"),
                 json_obj, Repid::ID_RESET_PWD,Moduls::RESETMOD);
}
