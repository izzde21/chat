    ...
    , ui(new Ui::AddFriend)
{
    ...
    connect(ui->more_lb,&ClickedOnceLabel::clicked,this,&AddFriend::Slotmoretip);
    connect(ui->lb_ed,&CustomizeEdit::returnPressed,this,&AddFriend::Slotpressenter);
    connect(ui->lb_ed,&CustomizeEdit::textChanged,this,&AddFriend::Slottextchange);
    connect(ui->lb_ed,&CustomizeEdit::editingFinished,this,&AddFriend::Sloteditfinish);
    connect(ui->tip_lb,&ClickedOnceLabel::clicked,this,&AddFriend::Slottipclick);
    ...
    connect(ui->sure_btn,&Clickedbtn::clicked,this,&AddFriend::Slotsurebtnclick);
    connect(ui->cancel_btn,&Clickedbtn::clicked,this,&AddFriend::Slotcancelbtnclick);
    InitTipLbs();
}
void AddFriend::InitTipLbs(){
    int lines = 1;
    for(int i = 0; i < _tip_data.size(); i++){
        ...
        connect(lb, &cilcklabel::clicked, this, &AddFriend::SlotChangeFriendLabelByTip);
...
//把标签放在lb_edit上
void AddFriend::addlabel(QString name){
    ...
    connect(tmplabel, &FriendLabel::sig_close, this, &AddFriend::SlotRemoveFriendLabel);
...
void AddFriend::Slottextchange(const QString& text){
    if(text.isEmpty()){
        ui->input_tip_wid->hide();
        return;
    }
...
void AddFriend::Sloteditfinish(){
    ui->input_tip_wid->hide();
}

void AddFriend::Slottipclick(QString text){
...
void AddFriend::Slotsurebtnclick(){
    ...
    auto uid=UserMgr::GetInstance()->Getuserinfo()->_uid;
    QJsonObject jsonobj;
    jsonobj["uid"]=uid;
    jsonobj["applyname"]=name;
    jsonobj["bakname"]=backed;
    jsonobj["touid"]=_si->_uid;
    qDebug() << "asdasdasdassd uid is asdasdasdasdas"<<UserMgr::GetInstance()->Getuserinfo()->_uid;
    QJsonDocument doc(jsonobj);
    QByteArray  jsonData =doc.toJson(QJsonDocument::Compact);
    emit TcpMgr::GetInstance()->sig_send_meg(ID_ADD_FRIEND_REQ,jsonData);
    this->hide();
    deleteLater();
}

void AddFriend::Slotcancelbtnclick(){
    this->hide();
    deleteLater();
}
...
//把标签放在label_list上
void AddFriend::AddTipLBS(cilcklabel* lb,QPoint cur_point,QPoint &next_point,int text_width,int text_hight){
    lb->move(cur_point);
    lb->show();
...
void AddFriend::SetSearchInfo(std::shared_ptr<SearchInfo> si){
    _si=si;
}

void AddFriend::resetLabels()
{
    auto max_width = ui->gridWidget->width();
    auto label_height = 0;
...
void AddFriend::SlotChangeFriendLabelByTip(QString lbtext, ClickLbState state){
...
void AddFriend::SlotRemoveFriendLabel(QString name)
{
    _label_point.setX(2);
    _label_point.setY(6);
