...
int main()
{
    ...
    try {
        ...
        std::string gate_port_str = conmgr["GateServer"]["Port"];
        unsigned short port = atoi(gate_port_str.c_str());
        net::io_context ioc;
        net::signal_set signal(ioc, SIGINT, SIGTERM);
        signal.async_wait([&](boost::system::error_code e,int um) {
            if (e) {
                return;
            }
            ioc.stop();
            });
        std::make_shared<CServe>(ioc, port)->Start();
        std::cout << "start receive port:" <<port<< std::endl;
        ioc.run();
    }
